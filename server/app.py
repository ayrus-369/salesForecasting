...
mongoDB=pymongo.MongoClient(mongoDB_url) #creating new client instance
db=mongoDB['mongodb']
login=db.mongocollection #holds the references to the mongocollection
app = Flask(__name__)
CORS(app)
...
def upload_file():
	...
	# plot predictions and actual values
		predictions.plot(legend = True,title="Actual Vs Prediction",x="month",y="Passengers")
	...
	# Calculate mean squared error
		...
		forecast = result.predict(start = len(bus),
							end = (len(bus)) +int(int( number)* x) ,
		...
		forecast.plot(legend = True,title="Forecasted Result For "+number+" "+value,markersize=15)
			
		
		final_data=pd.concat([test['BusPassenger'],forecast,predictions],axis=1)
		...
		plt.close()
		final_data.to_csv('F:/SalesForecasting/salesForecast/src/assets/result/forecast.csv',index_label='Date')	
